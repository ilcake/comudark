...
<link href='resources/myfiles/css/shared.css' rel='stylesheet'
	type='text/css'>
	...
	<div id="totalWrapper" style="background: none;">
		...
		<div class="wrapper">
			<div class="writeButton">
				<a href="write"><span class="glyphicon glyphicon-pencil"
					aria-hidden="true" boardnum="${board.boardnum}"
					filenum="${board.filenum}" style="color: white;"></span></a>
			</div>
			<div class="search" style="display: inline-box;">
				<form action="searchBoard" method="GET">
					<select name="searchTitle" class="searchTitle"><option
							value="userid">ID</option>
						<option value="title">TITLE</option>
						<option value="content">CONTENT</option></select>
					<!-- <input type="hidden" name="searchTitle" value="title"> -->
					<input type="text" name="searchText" class="searchText">&nbsp;
					<label><input type="submit" style="display: none;"><span
						class="glyphicon glyphicon-search" aria-hidden="true"
						style="color: white; padding-top: -10px;"></span></label>
				</form>
			</div>
			<div></div>
			<div></div>
							...
							<table class='board'>
								<tr>
									<td class="td_img">
										<div class="card-container tool">
											<div class="card">
												<div class="side">
													<img src="resources/profiles/${board.profile}"
														onERROR="this.src='images/user.png'" alt="image"
														class="profileImage">
												</div>
												<div class="side back">
													<img src="images/galaxy-2.jpg"
														style="position: absolute; left: 0;">
												</div>
											</div>
											<span class="toolt"><span class='pro hover'
												userid="${board.userid}">프로필</span><br>
											<span class="mes hover" userid="${board.userid}">메시지</span></span>
										</div>
									</td>
									...
									<!-- 좋아요/구독 버튼 -->
									<!-- 작성 ID가 로그인 ID와 같으면 표시하지 않음 -->
									<td class="td_button"><c:if
											test="${loginId != board.userid}">
								...
								<tr>
									...
									<td colspan="3" style="word-break: break-all;" class="content">
										...
										<!-- FILE --> <c:if test="${board.file_title != null }">
											<button class="buttonEffect" boardnum="${board.filenum}" filenum="${board.filenum}">
												<!-- <div class="fileField" style=""> -->
												<img class="fileImage"
											...
											<br>
											<br>
										</c:if> ${fn:replace(board.content, crcn, br)} <br> <!-- 글 수정,삭제 버튼 : 작성ID와 로그인ID가 다를 때만 표시 -->
										<c:if test="${board.userid == loginId}">
											<span class="writerButton"
												style="float: right; padding-right: 8px;"> <a
												href="selectBoard?boardnum=${board.boardnum}"><span
													class="glyphicon glyphicon-pencil" aria-hidden="true"
													boardnum="${board.boardnum}" style="font-size: small;"></span></a>
												<a href=""><span class="deleteBoard"
													boardnum="${board.boardnum}">X</span></a>
											</span>
										</c:if> <%-- 			<span class="replybutton" id="replybutton"> <a role="button"
								...
								<c:forEach var="reply" items="${replyAll}">
									<c:if test="${reply.boardnum == board.boardnum}">
										<tr class="reply">
											...
											<td style="width: 70%;">${reply.replytext}<span
												style="color: gray;"> ${reply.inputdate}</span></td>
											<td><c:if test="${reply.userid == loginId}">
													<!-- 리플ID와 loginID가 같을 때만 표시 -->
													<span class="glyphicon glyphicon-pencil updatereply"
														aria-hidden="true" style="font-size: small;"
														boardnum="${board.boardnum}" replynum="${reply.replynum}"
														replytext="${reply.replytext}"></span>
								...
								<tr>
									<form action="replyWrite" method="post">
										...
										<td class="td_center" style="width: 70%;"><input
											type="text" name="replytext" class="replytext"
											boardnum="${board.boardnum}" style="width: 100%;"></td>
										<td>
											<button style="border: none; background: none;" class="reply"
