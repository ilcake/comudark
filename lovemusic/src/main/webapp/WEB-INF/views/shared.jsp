...
<body>
	...
	<div id="totalWrapper" style="background: none;">
		...
		<div class="wrapper">
			<div class="search"
				style="height: 38px; color: white; top: 100px; right: 20px; margin: auto; position: absolute; border-radius: 10px; border: solid 2px red; padding: 2px; padding-bottom: 10px; padding-right: 10px;">
				<input type="text"
					style="background: none; border: none; color: white; padding-top: -20px; margin-top: -10px;">&nbsp;
				<a href="search"><span class="glyphicon glyphicon-search"
					aria-hidden="true" style="color: white; padding-top: -10px;"></span></a>
			</div>
			<div></div>
			<div></div>
			<hr>
			...
			<hr>
			<div id="container"
				style="margin: 30px; padding: 30px; padding-top: 50px;">
						...
						<table class='board'>
							...
							<tr>
								...
								<td colspan="3" style="word-break: break-all;" class="content">
									<div class="fileField"
										style="background-color: #f0f5f5; margin-top: -30px; margin-bottom: 10px; padding: 10px; border: dotted 0.5px white;">
										<img src="resources/covers/${board.cover_re}">
										&nbsp;&nbsp; File Title ${board.filenum}
									</div> ${fn:replace(board.content, crcn, br)} <br> <!-- 글 수정,삭제 버튼 : 작성ID와 로그인ID가 다를 때만 표시 -->
									<c:if test="${board.userid == loginId}">
										<span class="writerButton"
											style="float: right; padding-right: 8px;"> <a
											href="write?boardnum=${board.boardnum}"><span
							...
							<tr>
								<form action="replyWrite" method="get">
									...
									<td><button style="border: none; background: none;"
											class="reply" boardnum="${board.boardnum }">
